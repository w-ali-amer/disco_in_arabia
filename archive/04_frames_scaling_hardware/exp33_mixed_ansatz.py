# -*- coding: utf-8 -*-
"""Exp 33: mixed CRz/CRx verb entanglers — recombining the dissociated
properties.
...
if os.environ.get("ARABIC_POS_FUSION", "0") != "1":
    print("[33] WARNING: ARABIC_POS_FUSION is off — frame supply will be "
          "starved; exp33 is designed to run with fusion on.", flush=True)
MAX_TRY_PER_VERB = 24
MAX_USE_PER_VERB = 10
MIN_FRAMES = 4
N_NULL = 300
VARIANTS = ["zz", "zx", "xz", "zx4"]
...
print(f"[33] parsing {len(all_sents)} candidate frames for "
...
for k, fs in CAND.items():
    ...
    for sent, subj, split in fs:
        ...
        if len(gs) >= MAX_USE_PER_VERB:
            break
    print(f"[33] {surface[k]}: usable frames {len(gs)}/{len(fs)} attempted",
          flush=True)
...
for variant in VARIANTS:
    ...
    for k, gs in USABLE.items():
        ...
        print(f"[33] {variant} {v}: M={len(gs)} d={dim} | indiv "
    ...
    print(f"[33] {variant} SUMMARY: specificity={np.mean(spec):.4f} "
...
json.dump(OUT, open(f"results_exp33_s{S_OUT}.json", "w"), indent=2,
          ensure_ascii=False)
print("[33] DONE", flush=True)
