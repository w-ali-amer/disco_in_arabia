...
print(f"[22b] twins: {len(twins)}", flush=True)
...
def circuits_for(n_layers, n_s):
    ansatz = exp13.IQPAnsatz({exp13.S_ty: n_s, exp13.N_ty: 1},
                             n_layers=n_layers, discard=True, n_ancillas=1)
    circs, vidx = [], []
...
def dms_for(circs, wmap, dim):
    ...
    for c in circs:
        ...
        out.append(rho)
        traces.append(tr)
    print(f"[22b]   ({n_mixed}/{len(circs)} circuits mixed, rest pure)",
          flush=True)
    return out, traces
...
def analyse(rhos, A, B, C, tag):
    ...
    print(f"[22b] {tag}: A={out['medians']['A']:.4f} B={out['medians']['B']:.4f} "
...
for n_s in (1, 2):
    ...
    print(f"[22b] n_s={n_s}: {len(circs)} discard circuits", flush=True)
    ...
    if n_s == 2 and AE_MAP:
        ...
        print(f"[22b] ae_init name matches: {n_hit}/{len(names)}", flush=True)
    ...
    print(f"[22b] n_s={n_s} L0-discard control min fidelity = {f0:.12f}",
          flush=True)
    OUT[f"n_s_{n_s}"] = res
    json.dump(OUT, open("results_exp22b.json", "w"), indent=2)

OUT["runtime_sec"] = round(time.time() - t0, 1)
json.dump(OUT, open("results_exp22b.json", "w"), indent=2)
print(f"[22b] DONE in {OUT['runtime_sec']}s", flush=True)
