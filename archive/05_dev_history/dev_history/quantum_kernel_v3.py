# quantum_kernel_v2.py (Enhanced Version - Adjusted for Diagram-First Return)
# -*- coding: utf-8 -*-
...
from qiskit import Aer, execute, QuantumCircuit, ClassicalRegister # Ensure ClassicalRegister is imported
...
from camel_test import arabic_to_quantum_enhanced # Dependency
...
class ArabicQuantumMeaningKernel:
    ...
    def get_circuit_features(self, circuit, shots: int = 1024) -> np.ndarray:
        ...
        if not isinstance(circuit, QuantumCircuit):
             # This error check is crucial and will now be hit correctly after fixing the input type
                    ...
                    try:
                        ...
                        for i in range(num_pauli_checks):
                            ...
                            if not meas_circuit.cregs:
                                ...
                                # If existing register is too small, add a new one
                            ...
                            if i < cr.size: meas_circuit.measure(i, cr[i]) # Measure qubit i into classical bit i
                            ...
                            if total > 0:
                                 ...
                                 for bitstring, count in counts.items():
                                      # Qiskit bitstrings are often read right-to-left (least significant bit first)
                                      # ClassicalRegister bits map directly, so cr[i] corresponds to the i-th position
                                      bit_index = i
                                      if 0 <= bit_index < len(bitstring):
                                          # Check the bit at the index corresponding to the measured qubit
                                          if bitstring[len(bitstring) - 1 - bit_index] == '0': # Adjust index for Qiskit's ordering
                                              prob_0 += count / total
                                          else:
                                              prob_1 += count / total
    ...
    def extract_complex_linguistic_features(self, tokens, analyses, structure, roles):
        ...
        # Find the index of the next available feature slot
    ...
    def train(self, sentences, circuits, tokens_list, analyses_list, structures, roles_list, use_enhanced_clustering=False):
        ...
        for i in range(min_len):
            ...
            try:
                # Ensure we are passing the Qiskit circuit object
                ...
                if not isinstance(current_circuit, QuantumCircuit):
                     ...
                     continue # Skip this sentence if the circuit object is incorrect
        ...
        # --- MODIFIED: Choose clustering method ---
        if use_enhanced_clustering and GENSIM_AVAILABLE:
            print("Learning meaning clusters (enhanced with topic modeling)...") # DEBUG
            self.learn_enhanced_meaning_clusters(embeddings, sentences) # Call new function
        else:
            ...
            print("Learning meaning clusters (basic)...") # DEBUG
            self.learn_meaning_clusters(embeddings) # Call original function
        # --- END MODIFICATION ---
    ...
    def learn_enhanced_meaning_clusters(self, embeddings, sentences):
        ...
        try:
            ...
            # Simple split, consider using CAMeL tokenizer if available for better results
            tokenized_sentences = [sentence.split() for sentence in sentences]

            # Remove common Arabic stopwords (customize this list)
            arabic_stopwords = ['من', 'في', 'على', 'الى', 'إلى', 'عن', 'و', 'ف', 'ثم', 'أو', 'لا',
                                'ما', 'هو', 'هي', 'هم', 'هن', 'هذا', 'هذه', 'ذلك', 'تلك', 'الذي',
                                'التي', 'الذين', 'قد', 'لقد', 'أن', 'ان', 'إن', 'كان', 'يكون', 'لم',
                                'لن', 'كل', 'بعض', 'يا', 'اي', 'أي', 'مع', 'به', 'له', 'فيه', 'تم']
            filtered_sentences = [[word for word in sentence if word not in arabic_stopwords and len(word) > 1]
            ...
            # Number of topics - adjust as needed, ensure it's less than num features
            ...
            lda_model = models.LdaModel(corpus=corpus,
                                    ...
                                    passes=15, # Increase passes for potentially better results
                                    random_state=42)
            ...
            if not self.meaning_map: # Ensure meaning_map is initialized
                 # Need to call assign_meaning_to_clusters first if it hasn't been called
                 # This requires access to structures, roles_list, analyses_list
                 # For simplicity, we assume assign_meaning_to_clusters will be called after this in train()
                 ...
                 pass # Or call assign_meaning_to_clusters here if data is available

            for cluster_id in range(self.num_clusters): # Iterate through expected cluster IDs
                ...
                # Get top words for the most dominant topics in the cluster
                topic_words = {}
                # Get top 3 topics for the cluster
                top_cluster_topics = cluster_topic_dist.most_common(3)
                for topic_id, total_prob in top_cluster_topics:
                     # Get top 5 words for this topic_id from the LDA model
                     words = lda_model.show_topic(topic_id, topn=5)
                     topic_words[f"Topic {topic_id}"] = {'words': [word for word, _ in words], 'weight': total_prob}

                # Add topic information to meaning map (ensure cluster_id exists)
                if cluster_id in self.meaning_map:
                    self.meaning_map[cluster_id]['topics'] = topic_words
                else:
                    # Initialize the map entry if assign_meaning_to_clusters hasn't run yet
                    self.meaning_map[cluster_id] = {'topics': topic_words}
                    # print(f"Warning: Initializing meaning_map entry for Cluster ID {cluster_id} during topic assignment.")
    ...
    def assign_meaning_to_clusters(self, sentences: List[str], structures: List[str], roles_list: List[Dict], analyses_list: List[List[Tuple]]) -> Dict:
        ...
        # self.meaning_map = {} # Don't reset if called after enhanced clustering
        ...
        for cluster_id, cluster_data in cluster_data_grouped.items():
            ...
            for item in cluster_data:
                ...
                if self.camel_analyzer: # Use CAMeL if available
                    try:
                        # Analyze the full sentence string
                        morph_analysis_list = self.camel_analyzer.analyze(item['sentence'])
                        # Find the analysis for the verb token
                        if verb_idx is not None and verb_idx < len(morph_analysis_list):
                            verb_morph_analyses = morph_analysis_list[verb_idx] # This is a list of possible analyses
                            if verb_morph_analyses: # If the list is not empty
                                verb_morph = verb_morph_analyses[0] # Take the first analysis
                                verb_tenses[verb_morph.get('asp', 'UNK')] += 1
                                verb_moods[verb_morph.get('mod', 'UNK')] += 1
                    except Exception as e: pass # print(f"CAMeL analysis failed for cluster {cluster_id}, sentence '{item['sentence'][:30]}...': {e}") # DEBUG
            ...
            # --- Store results (Update existing entry or create new) ---
            if cluster_id not in self.meaning_map:
                 self.meaning_map[cluster_id] = {} # Initialize if not present
            ...
            self.meaning_map[cluster_id].update({
                ...
                # 'topics' field is handled by learn_enhanced_meaning_clusters if used
            })
            if 'topics' not in self.meaning_map[cluster_id]:
                 self.meaning_map[cluster_id]['topics'] = {} # Ensure topics field exists
    ...
    def create_specific_interpretation(self, tokens: List[str], analyses: List[Tuple], roles: Dict, structure: str, templates: Dict) -> Dict:
        ...
        if self.camel_analyzer and verb_idx is not None: # Check if analyzer exists
             try:
                 ...
                 # Get the analysis for the specific verb token
                 if verb_idx < len(morph_analysis_list):
                     verb_analysis_list = morph_analysis_list[verb_idx] # List of possible analyses
             ...
             except Exception as e: pass # print(f"      CAMeL analysis error in specific interpretation: {e}") # DEBUG
        ...
        semantic_details = {
            ...
            'semantic_frames': semantic_frames # Basic frames added here
        }
    ...
    def analyze_sentence_in_context(self, current_circuit, current_tokens, current_analyses, current_structure, current_roles, previous_analysis_dict=None):
        ...
        # IMPORTANT: Call interpret_sentence with previous_analyses=None to avoid recursion loop
        base_interpretation = self.interpret_sentence(current_circuit, current_tokens, current_analyses, current_structure, current_roles, previous_analyses=None) # Ensure no context is passed here
        ...
        # Optional: Contextual Embedding Adjustment (Example - Simple Averaging)
    ...
    def extract_semantic_frames(self, tokens, analyses, roles):
        ...
        (Adapted from quantum_kernel.py - requires ARAVEC_MODEL, FARASA_NER globals)
        """
        ...
        if verb_idx is not None and verb_idx < len(tokens):
            ...
            if verb_class != "UNKNOWN" and verb_class != "ACTION":
                ...
                # Assign typical roles based on verb class
        ...
        if self.camel_analyzer:
            try:
                ...
                semantic_properties = {
                    'tense': None, 'mood': None, 'aspect': None,
                    'definiteness': [], 'gender': {}, 'number': {}
                }
                for i, token_analysis_list in enumerate(camel_analysis):
                    if token_analysis_list: # Check if list is not empty
                        token_analysis = token_analysis_list[0] # Take first analysis
                        ...
                        if 'def' in token_analysis and token_analysis['def'] == 'd': # Check for definite 'd'
                ...
                print(f"Warning: Error in CAMeL semantic property extraction: {e}")
        ...
        if FARASA_NER:
            try:
                ...
                if entities:
                    frames.append({'type': 'NAMED_ENTITIES', 'entities': entities})
            except Exception as e:
                print(f"Warning: Error using Farasa NER: {e}")
        ...
        if ARAVEC_MODEL:
            try:
                ...
                for i, (lemma, pos, _, _) in enumerate(analyses):
                    if pos in ['NOUN', 'VERB', 'ADJ'] and lemma in ARAVEC_MODEL.wv: # Check vocab
                        similar = ARAVEC_MODEL.wv.most_similar(lemma, topn=3)
                        similar_words[lemma] = [word for word, _ in similar]
                if similar_words:
                    frames.append({'type': 'WORD_EMBEDDINGS', 'similar_words': similar_words})
            except Exception as e:
                 print(f"Warning: Error using AraVec model: {e}")
    ...
    def extract_enhanced_semantic_frames(self, tokens, analyses, roles):
        ...
        # 1. Extract rhetorical relations (cause, contrast, condition, etc.)
        rhetorical_markers = {
            'CAUSE': ['لأن', 'بسبب', 'نتيجة', 'لذلك'],
            'CONTRAST': ['لكن', 'بينما', 'رغم', 'مع ذلك', 'على الرغم', 'إلا أن', 'غير أن'],
            'CONDITION': ['إذا', 'لو', 'إن', 'شرط', 'في حال'],
            'TEMPORAL': ['قبل', 'بعد', 'أثناء', 'خلال', 'عندما', 'حين'],
            'PURPOSE': ['كي', 'ل', 'من أجل', 'بهدف', 'حتى'],
            'CONJUNCTION': ['و', 'ف', 'ثم'] # Basic conjunctions
        }

        for rel_type, markers in rhetorical_markers.items():
            for i, token in enumerate(tokens):
                if token in markers:
                    # Simple span identification (can be improved)
                    span1_tokens = tokens[:i]
                    span2_tokens = tokens[i+1:]
                    # Avoid adding empty spans
                    if span1_tokens and span2_tokens:
                        frames.append({
                            'type': f'RHETORICAL_{rel_type}',
                            'marker': token,
                            'span1': ' '.join(span1_tokens),
                            'span2': ' '.join(span2_tokens)
                        })
                        # Break if marker found to avoid multiple relations for same marker (simplification)
                        # break # Commented out to allow multiple markers like 'و'

        # 2. Extract nested predications (clauses within clauses) - Simplified
        verb_indices = [i for i, (_, pos, _, _) in enumerate(analyses) if pos == 'VERB']
        if len(verb_indices) > 1:
            # Find root verb
            root_idx = roles.get('root')
            main_verb_idx = roles.get('verb', root_idx)

            if main_verb_idx is not None:
                 subordinate_clauses = []
                 for i, (_, pos, dep, head) in enumerate(analyses):
                      # Look for clausal complements or adverbial clauses linked to main verb
                      if pos == 'VERB' and i != main_verb_idx and (dep in ['ccomp', 'xcomp', 'advcl'] or head == main_verb_idx):
                           # Simple span: from this verb to end of sentence (needs improvement)
                           clause_span = ' '.join(tokens[i:])
                           subordinate_clauses.append({
                               'verb': tokens[i],
                               'span': clause_span,
                               'index': i,
                               'relation_to_main': dep
                           })
                 if subordinate_clauses:
                      frames.append({
                          'type': 'NESTED_PREDICATION',
                          'main_verb': tokens[main_verb_idx] if main_verb_idx < len(tokens) else '?',
                          'main_index': main_verb_idx,
                          'subordinate_clauses': subordinate_clauses
                      })

        # 3. Extract coreference chains (Simple pronoun resolution)
        pronouns = ['هو', 'هي', 'هم', 'هن', 'ه', 'ها', 'هما', 'ك', 'كم', 'نا'] # Add more pronouns
        pronoun_indices = [(i, token) for i, token in enumerate(tokens) if token in pronouns]

        coref_chains = []
        processed_pronouns = set() # To avoid duplicate chains for same pronoun mention
        for i, (pronoun_idx, pronoun) in enumerate(pronoun_indices):
            if pronoun_idx in processed_pronouns: continue

            # Find potential antecedents (nouns/proper nouns before the pronoun)
            potential_antecedents = []
            for k in range(pronoun_idx - 1, -1, -1): # Search backwards
                if k < len(analyses):
                    lemma, pos, _, _ = analyses[k]
                    if pos in ['NOUN', 'PROPN']:
                        # Basic agreement check (can be enhanced with CAMeL gender/number)
                        potential_antecedents.append({'text': tokens[k], 'index': k, 'lemma': lemma})
                        # Simple heuristic: take the closest one for now
                        break
            if potential_antecedents:
                antecedent = potential_antecedents[0] # Closest preceding noun/proper noun
                coref_chains.append({
                    'pronoun': pronoun,
                    'pronoun_index': pronoun_idx,
                    'antecedent': antecedent['text'],
                    'antecedent_index': antecedent['index']
                })
                processed_pronouns.add(pronoun_idx)


        if coref_chains:
            frames.append({'type': 'COREFERENCE', 'chains': coref_chains})
    ...
    def interpret_sentence(self, circuit, tokens: List[str], analyses: List[Tuple], structure: str, roles: Dict, previous_analyses=None) -> Dict:
        ...
        quantum_features = self.get_enhanced_circuit_features(circuit) # This will now receive the correct type or error out if fix wasn't applied
        ...
        if self.camel_analyzer:
             ...
             try:
                 ...
                 # Example (requires CAMeL sentiment/NER components installed and loaded):
                 # try:
             ...
             except Exception as camel_e: pass # print(f"    Error during CAMeL analysis: {camel_e}") # DEBUG
        ...
        # --- MODIFIED: Extract Semantic Frames ---
        # print("  Extracting semantic frames...") # DEBUG
        try:
            ...
            # print(f"    Extracted {len(extracted_frames)} frames.") # DEBUG
        ...
        result = {
            ...
            'discourse_relations': [], # Discourse relations added by context analysis wrapper
    ...
    # --- NEW: Markdown Report Function (from quantum_kernel.py) ---
    def generate_discourse_report(self, discourse_analyses):
        ...
        for i, analysis in enumerate(discourse_analyses):
            ...
            interpretation_data = analysis.get('interpretation', {}) # Dict from interpret_sentence

            report += f"## Sentence {i+1}\n"
            report += f"**Text:** `{sentence}`\n\n"

            # Handle potential error during interpretation
            ...
            if interpretation_data.get('sentiment') is not None:
                report += f"- Sentiment: {interpretation_data['sentiment']}\n"
            if interpretation_data.get('named_entities'):
                report += f"- Named Entities: `{interpretation_data['named_entities']}`\n"
            report += "\n"
            ...
            if specific_interp and isinstance(specific_interp, dict):
                ...
                if sem_details:
                     ...
                     report += f"  - Verb: `{verb_info.get('text', '-')}` (Lemma: `{verb_info.get('lemma', '-')}`, Tense: {verb_info.get('tense', '-')})\n"
                     report += f"  - Subject: `{sem_details.get('subject', {}).get('text', '-')}`\n"
                     report += f"  - Object: `{sem_details.get('object', {}).get('text', '-')}`\n"
            else:
                report += f"- General Interpretation: {interpretation_data.get('interpretation', 'N/A')}\n"
            report += "\n"
            ...
            if frames:
                for frame in frames:
                     frame_type = frame.get('type', 'UNKNOWN_FRAME')
                     details = ', '.join([f"{k}: {v}" for k, v in frame.items() if k != 'type'])
                     report += f"- Type: **{frame_type}**\n"
                     # Nicer formatting for specific frame types
                     if frame_type == 'SEMANTIC_PROPERTIES':
                          props = frame.get('properties', {})
                          report += f"  - Properties: Tense=`{props.get('tense')}`, Aspect=`{props.get('aspect')}`, Mood=`{props.get('mood')}`\n"
                     elif frame_type == 'NAMED_ENTITIES':
                          ents = frame.get('entities', [])
                          report += f"  - Entities: `{ents}`\n"
                     elif frame_type == 'COREFERENCE':
                          chains = frame.get('chains', [])
                          report += f"  - Coref Chains: `{chains}`\n"
                     elif frame_type.startswith('RHETORICAL_'):
                          report += f"  - Marker: `{frame.get('marker')}`, Span1: `{frame.get('span1')}`, Span2: `{frame.get('span2')}`\n"
                     else: # Generic frame
                          report += f"  - Details: {details}\n"
            else:
                report += "- No specific semantic frames extracted.\n"
            report += "\n"
            ...
            if i > 0: # Only show relations for sentences after the first one
                ...
                relation_text = self.format_discourse_relations(discourse_relations) # Use existing formatter
                report += f"{relation_text}\n\n"
            else:
                report += "**Relationship to Previous Sentence:**\n- N/A (First Sentence)\n\n"
    ...
    def generate_html_report(self, discourse_analyses):
        ...
        <head>
            <meta charset="UTF-8">
            <title>تحليل الخطاب الكمي للغة العربية (محسن)</title>
            <style>
        ...
        <body>
            <h1>تحليل الخطاب الكمي للنص العربي (محسن)</h1>
        """
        for i, analysis in enumerate(discourse_analyses):
            sentence = analysis.get('sentence', 'N/A')
            ...
            # Handle potential error during interpretation
            if interpretation_data.get('error'):
                 html += f'<div class="analysis-section error">'
                 html += f'<h3>خطأ في التحليل:</h3>'
                 html += f'<div class="analysis-detail">{interpretation_data["error"]}</div>'
                 html += '</div>'
                 html += '</div>' # Close sentence-block
                 continue # Skip the rest for this sentence
            ...
            if interpretation_data.get('named_entities'):
                 # Format NEs nicely
                 ne_html = ", ".join([f"<code>{token} ({tag})</code>" for token, tag in interpretation_data["named_entities"]])
                 html += f'<div class="analysis-detail"><span class="detail-label">الكيانات المسماة:</span> {ne_html}</div>'
            html += '</div>'
            ...
            if specific_interp and isinstance(specific_interp, dict):
                ...
                if sem_details:
                     verb_info = sem_details.get('verb', {})
                     subj_info = sem_details.get('subject', {})
                     obj_info = sem_details.get('object', {})
                     html += f'<div class="analysis-detail" style="font-size: 0.9em; color: #444;">'
                     html += f' <span class="detail-label">الفعل:</span> <code>{verb_info.get("text", "-")}</code> (Lemma: <code>{verb_info.get("lemma", "-")}</code>, الزمن: {verb_info.get("tense", "-")})'
                     html += f' | <span class="detail-label">الفاعل:</span> <code>{subj_info.get("text", "-")}</code>'
                     html += f' | <span class="detail-label">المفعول:</span> <code>{obj_info.get("text", "-")}</code>'
                     html += f'</div>'
                html += '</div>'
            else:
            ...
            # --- MODIFIED: Display Semantic Frames ---
            frames = interpretation_data.get('semantic_frames', [])
            if frames:
                html += '<div class="analysis-section frames">'
                html += '<h3>الإطارات الدلالية:</h3>'
                for frame in frames:
                    frame_type = frame.get('type', 'UNKNOWN_FRAME')
                    html += f'<div class="frame-item"><span class="frame-type">{frame_type}:</span> '
                    details = []
                    for k, v in frame.items():
                        if k != 'type':
                             # Make lists/dicts slightly more readable
                             v_str = str(v)
                             if len(v_str) > 100: v_str = v_str[:100] + "..."
                             details.append(f'<span class="detail-label">{k}=</span><code>{v_str}</code>')
                    html += ", ".join(details)
                    html += '</div>'
                html += '</div>'
            # --- END MODIFICATION ---
            ...
            if i > 0: # Only show relations for sentences after the first one
                ...
                if discourse_relations:
                     for line in relation_text.split('\n'):
                          # Highlight marker in HTML
                          line_html = line.replace("'", "<code>").replace("'", "</code>", 1) # Basic marker highlighting
                          html += f'<div class="relation">{line_html}</div>'
                else:
    ...
    def save_model(self, filename: str = 'arabic_quantum_kernel.pkl'):
        ...
        # Ensure CAMeL analyzer is not saved directly if it's not pickleable
        analyzer_state = None
        if hasattr(self.camel_analyzer, '__getstate__'): # Check if it supports pickling state
        ...
        model_data = {
            ...
            # 'camel_analyzer_state': analyzer_state # Optionally save state if possible
        }
        try:
    ...
    def load_model(self, filename: str = 'arabic_quantum_kernel.pkl'):
        ...
        try:
            ...
            # Re-initialize CAMeL Analyzer instead of loading state (safer)
            # analyzer_state = model_data.get('camel_analyzer_state')
            # if analyzer_state:
            #     try:
            #         # Re-initialize first
            #         from camel_tools.morphology.database import MorphologyDB
            #         from camel_tools.morphology.analyzer import Analyzer
            #         db_path = MorphologyDB.builtin_db('calima-msa-r13') # Use same DB as init
            #         self.camel_analyzer = Analyzer(db_path)
            #         # Then set state if possible
            #         if hasattr(self.camel_analyzer, '__setstate__'):
            #              self.camel_analyzer.__setstate__(analyzer_state)
            #              print("CAMeL Analyzer state restored.")
            #         else:
            #              print("CAMeL Analyzer re-initialized, but state could not be restored.")
            #     except Exception as e:
            #          print(f"Warning: Failed to restore CAMeL Analyzer state: {e}")
            # else:
            #      # Try to initialize normally if no state saved
    ...
    def visualize_meaning_space(self, highlight_indices=None, save_path=None):
        ...
        # Check for NaNs or Infs before PCA
        finite_mask = np.all(np.isfinite(X), axis=1)
        if not np.all(finite_mask):
             print("Warning: Non-finite values (NaN or Inf) found in embeddings. Attempting to remove problematic rows.")
             X = X[finite_mask]
             # Filter indices and potentially cluster labels if they exist
             original_indices = indices
             indices = [idx for i, idx in enumerate(original_indices) if finite_mask[i]]
             if self.cluster_labels is not None:
                  if len(self.cluster_labels) == len(finite_mask):
                       self.cluster_labels = self.cluster_labels[finite_mask]
                       print(f"Filtered cluster labels to size {len(self.cluster_labels)}")
                  else:
                       print("Warning: Cluster labels length mismatch after filtering NaNs. Cannot use labels for coloring.")
                       self.cluster_labels = None # Invalidate labels if length mismatch

             if X.shape[0] == 0:
                  print("Error: All embeddings contained non-finite values. Cannot visualize.")
                  return None
        ...
        if X.shape[0] < 2: print("Warning: Need at least 2 samples for PCA visualization."); return None
        ...
        try:
            ...
            # Use cluster labels for color if available and matches filtered data
            current_labels = None
            if self.cluster_labels is not None and len(self.cluster_labels) == len(reduced_embeddings):
                 current_labels = np.array(self.cluster_labels)


            colors = current_labels if current_labels is not None else 'blue'
            cmap = 'viridis' if current_labels is not None else None
            ...
            if highlight_indices is not None:
                 # Map original highlight indices to the potentially filtered indices list
                 highlight_idxs_in_filtered = [indices.index(i) for i in highlight_indices if i in indices]
                 if highlight_idxs_in_filtered:
                    plt.scatter(reduced_embeddings[highlight_idxs_in_filtered, 0], reduced_embeddings[highlight_idxs_in_filtered, 1], c='red', s=150, edgecolor='white', zorder=10, label='Highlighted')
                    # Add labels (optional, can get crowded)
                    # for idx_f in highlight_idxs_in_filtered:
                    #     original_idx = indices[idx_f] # Get original index from filtered list
                    #     plt.annotate(self.reference_sentences.get(original_idx, f"Sent {original_idx}"), (reduced_embeddings[idx_f, 0], reduced_embeddings[idx_f, 1]), xytext=(5, 5), textcoords='offset points')
                 ...
                 if np.all(np.isfinite(self.meaning_clusters)):
                     try:
                         ...
                         if hasattr(pca, 'components_'):
                              ...
                              print("Warning: PCA not fitted, cannot transform cluster centers.")
            ...
            plt.title('Quantum Sentence Meaning Space (PCA)')
    ...
    # --- NEW: Quantum State Visualization (from quantum_kernel.py) ---
    def analyze_quantum_states(self, circuits_dict, save_path_prefix=None):
        ...
        Args:
            circuits_dict: Dictionary {id: circuit} of quantum circuits to analyze
            save_path_prefix: Base path prefix to save visualizations (e.g., "state_viz_")
        """
        if not circuits_dict:
            print("No circuits provided for state analysis.")
            return

        print(f"Analyzing quantum states for {len(circuits_dict)} circuits...")
        num_circuits = len(circuits_dict)
        # Adjust layout based on number of circuits
        ncols = 3
        ...
        for circuit_id, circuit in circuits_dict.items():
            if i >= len(axes): break # Should not happen with calculated nrows/ncols

            ax = axes[i]
            sentence_label = self.reference_sentences.get(circuit_id, f"Circuit {circuit_id}")
            if len(sentence_label) > 40: sentence_label = sentence_label[:37] + "..."

            try:
                # *** Check the type passed to this function ***
                if not isinstance(circuit, QuantumCircuit):
                    print(f"Skipping item {circuit_id}: Not a Qiskit QuantumCircuit (type: {type(circuit)})") # Error source
                    ax.text(0.5, 0.5, f"Invalid Circuit Object\n(Type: {type(circuit).__name__})\n(ID: {circuit_id})", ha='center', va='center', fontsize=9, color='red')
                    ax.set_title(f"Error: {sentence_label}", fontsize=10)
                    ax.set_xticks([])
                    ax.set_yticks([])
                    i += 1
                    continue
                ...
                if hasattr(result, 'get_statevector'):
                    ...
                    if save_path_prefix:
                         try:
                              ...
                              plot_state_city(statevector, title=f"State: {sentence_label}")
                              individual_path = f"{save_path_prefix}circuit_{circuit_id}.png"
                              individual_fig.savefig(individual_path, dpi=150, bbox_inches='tight')
                              plt.close(individual_fig) # Close the figure to free memory
                         except Exception as save_e:
                              print(f"Warning: Failed to save individual state plot for {circuit_id}: {save_e}")

                else:
                    ax.text(0.5, 0.5, "Statevector not available", ha='center', va='center', fontsize=9)
                    ax.set_title(sentence_label, fontsize=10)
                    ax.set_xticks([])
                    ax.set_yticks([])
        ...
        fig.suptitle("Quantum State Visualizations", fontsize=16)

        if save_path_prefix:
            try:
                 overview_path = f"{save_path_prefix}all_states_overview.png"
                 fig.savefig(overview_path, dpi=150)
                 print(f"Saved quantum states overview to {overview_path}")
            except Exception as overview_save_e:
                 print(f"Warning: Failed to save overview state plot: {overview_save_e}")
...
def prepare_quantum_nlp_pipeline(max_sentences=20, use_enhanced_clustering=True):
    ...
    results = []
    circuits_for_viz = {} # Store circuits with their original index for visualization
    print("\n--- Processing Sentences ---")
    for idx, sentence in enumerate(sentences):
        print(f"Processing sentence {idx+1}/{len(sentences)}: '{sentence[:40]}...'")
        try:
            # --- OPTION B APPLIED HERE: Assuming diagram is returned first ---
            # If you fixed camel_test.py (Option A), swap these back:
            # circuit, diagram, structure, tokens, analyses, roles = arabic_to_quantum_enhanced(sentence, debug=False)
            diagram, circuit, structure, tokens, analyses, roles = arabic_to_quantum_enhanced(sentence, debug=False) # Swapped!

            # Store the generated Qiskit circuit (now the second item) and other info
            results.append({
                'sentence': sentence,
                'circuit': circuit, # Store the circuit object
                'structure': structure,
                'tokens': tokens,
                'analyses': analyses,
                'roles': roles,
                'original_index': idx
                # Optionally store the diagram too if needed elsewhere
                # 'diagram': diagram
            })
            # *** Store the circuit object (now the second item) ***
            circuits_for_viz[idx] = circuit
            print(f"  Sentence {idx+1}: Processed successfully. Structure: {structure}, Circuit type: {type(circuit)}") # Should now show QuantumCircuit
        except Exception as e:
    ...
    kernel_clusters = min(5, max(1, num_processed // 2)) # Adjust cluster heuristic
    ...
    # Pass the list of circuits (which should now be correct Qiskit objects)
    kernel.train(
        sentences=[r['sentence'] for r in results],
        circuits=[r['circuit'] for r in results], # Pass the corrected circuits
        ...
        use_enhanced_clustering=use_enhanced_clustering # Pass the flag
    )

    # Save the model
    kernel.save_model('arabic_quantum_kernel_enhanced.pkl') # Use new name
    ...
    try:
        ...
        if meaning_space_fig: plt.close(meaning_space_fig) # Close figure
    except Exception as vis_e:
        print(f"Error during meaning space visualization: {vis_e}")


    # --- Remove Debugging Print (Optional) ---
    # print("\n--- Checking types before state visualization ---")
    # if circuits_for_viz:
    #     for k, v in circuits_for_viz.items():
    #         print(f"ID {k}: Type {type(v)}")
    # else:
    #     print("circuits_for_viz dictionary is empty.")
    # print("--- End type check ---")
    # --- End Remove Debugging Print ---


    # --- NEW: Visualize Quantum States ---
    print("\n--- Visualizing Quantum States ---")
    try:
        # Pass the dictionary of circuits {original_index: circuit}
        # This should now contain Qiskit circuits if the fix worked
        state_viz_fig = kernel.analyze_quantum_states(circuits_for_viz, save_path_prefix="state_viz_")
        if state_viz_fig: plt.close(state_viz_fig) # Close figure
    except Exception as state_vis_e:
        print(f"Error during quantum state visualization: {state_vis_e}")
    # --- END NEW ---
    ...
    for i, result in enumerate(results):
        ...
        try:
            # Pass the Qiskit circuit (result['circuit']) to interpret_sentence
            ...
            # Store full analysis including interpretation
            current_analysis_dict = {**result, 'interpretation': interpretation}
            discourse_analyses.append(current_analysis_dict)
            previous_analysis_dict = current_analysis_dict # Update context
            print(f"  Sentence {i+1}: Analysis complete.")
        except Exception as analysis_e:
             print(f"  Error during sentence analysis {i+1}: {analysis_e}")
             # Add more specific check for the type error if it persists
             if isinstance(result.get('circuit'), QuantumCircuit) == False:
                  print(f"  >>> Error likely due to incorrect circuit type ({type(result.get('circuit'))}) being passed from results list.")
             traceback.print_exc()
    ...
    except Exception as md_report_e:
        print(f"Error generating Markdown report: {md_report_e}")
    # --- END NEW ---
    ...
    try:
        ...
        # *** Apply the same swap here if using Option B ***
        new_diagram, new_circuit, new_structure, new_tokens, new_analyses, new_roles = arabic_to_quantum_enhanced(new_sentence, debug=False) # Swapped!

        # Interpret using the new Qiskit circuit and context from last processed sentence
        last_interpretation_result = kernel.interpret_sentence(
            new_circuit, new_tokens, new_analyses, new_structure, new_roles,
            previous_analyses=previous_analysis_dict # Use context
        )
