...
try:
    ...
    try:
        from qiskit_aer.primitives import Sampler, Estimator  # Aer V1-compatible primitives (Qiskit 2.x)
    except ImportError:
        from qiskit.primitives import Sampler, Estimator  # Fallback for Qiskit <2.0
...
try:
    from common_qnlp_types import (
        ...
        Sim14Ansatz, Sim15Ansatz,  # Hardware-efficient circuit ansatzes
    )
...
try:
    ...
    def cross_val_score(*args, **kwargs): return np.array([0.0])
...
class ArabicQuantumMeaningKernel:
    ...
    def __init__(self,
        ...
        if QISKIT_AVAILABLE:
            ...
            self.estimator = Estimator(run_options={'shots': self.shots})
        else:
    ...
    def _build_token_mapping(self,
        ...
        if diagram_main and hasattr(diagram_main, 'boxes'):
            for box in diagram_main.boxes:
                ...
                if analysis:
                    ...
                    token_map[box.name] = entry
                    # Also register lemma as key so lambeq param names (lemma__type_idx) match via startswith
                    lemma_key = analysis.get('lemma', analysis.get('text', ''))
                    if lemma_key and lemma_key not in token_map:
                        token_map[lemma_key] = entry
        ...
        if analyses and not token_map:
            for analysis in analyses:
                ...
                # Lambeq 0.4+ names parameters as "{word_name}__{type}_{idx}"
                # where word_name is the lemma passed to Word(). Use lemma directly
                # so startswith-matching in _categorize_parameters works correctly.
                lemma = analysis.get('lemma', analysis.get('text', ''))
                is_ambiguous = ambiguous_map and token_idx in ambiguous_map

                if is_ambiguous:
                    prefix = lemma  # e.g. "عين" matches "عين__n_0"
                    senses = ambiguous_map[token_idx].get('senses', [])
                else:
                    prefix = lemma  # e.g. "وَلَد" matches "وَلَد__n_0"
                    senses = []
        ...
        logger.info(f"Token map built: {len(token_map)} entries, source='{token_map.get(next(iter(token_map)), {}).get('source', 'none') if token_map else 'none'}', keys={list(token_map.keys())[:6]}")
        return token_map
    ...
    def _categorize_parameters(self, 
        ...
        logger.info(f"Parameter categorization: {len(categories['feature'])} feature (AraVec/core), "
                    f"{len(categories['variational'])} variational, {len(categories['unknown'])} unknown (hash-fallback). "
                    f"Token map keys: {list(token_map.keys())[:8]}")
    ...
    def _bind_feature_parameters(self,
        ...
        for param, param_info in feature_params:
            ...
            if feature_vec is not None and feature_vec.size > 0:
                ...
                # Fallback to hash if no features available (AraVec had no vector for this lemma)
                bound_params[param] = self._hash_to_angle(param.name)
                token_text = token_info.get('analysis', {}).get('lemma', '?') if token_info else '?'
                logger.warning(f"AraVec miss for param '{param.name}' (lemma='{token_text}'); using hash fallback")
    ...
    def get_enhanced_circuit_features(self,
        ...
        try:
            parameter_binds_map = self._bind_parameters(
                circuit,
                None,  # diagram_main — not available at this call site; analysis-based fallback used
                tokens_from_main_processor,
                analyses_from_main_processor,
                linguistic_stream_core
            )
    ...
    def get_aravec_sentence_embedding(self, tokens: List[str], analyses: List[Dict]) -> Optional[np.ndarray]:
        """
        Compute a raw AraVec sentence embedding by averaging token vectors.
        Tries surface tokens first, then lemmas from analyses.
        Returns normalized 300-dim vector, or None if no tokens found in vocab.
        """
        if not hasattr(self, 'embedding_model') or self.embedding_model is None:
            return None
        vecs = []
        for tok in tokens:
            if self.embedding_model.has_index_for(tok):
                vecs.append(self.embedding_model.get_vector(tok))
        # Fallback to lemmas if surface tokens miss
        if not vecs and analyses:
            for ana in analyses:
                lemma = ana.get('lemma', '')
                if lemma and self.embedding_model.has_index_for(lemma):
                    vecs.append(self.embedding_model.get_vector(lemma))
        if not vecs:
            return None
        avg = np.mean(vecs, axis=0).astype(np.float64)
    ...
    def combine_features_with_attention(self, quantum_features, linguistic_features, structure,
        ...
        """
        Combine quantum Pauli embeddings with raw AraVec sentence mean via concatenation.
        This replaces the broken weighted blend (which destroyed signal from both channels).
        Result: [quantum_features_normalized | aravec_mean_normalized] concatenated.
        Falls back to [quantum | linguistic_morph_features] if AraVec unavailable.
        """
        # Normalize quantum features
        q = np.array(quantum_features, dtype=np.float64)
        q_norm = np.linalg.norm(q)
        q = q / q_norm if q_norm > 1e-9 else q
        if not np.all(np.isfinite(q)):
            q = np.zeros(self.embedding_dim)

        # Try raw AraVec sentence mean (the meaningful classical signal)
        aravec_vec = None
        if tokens is not None and analyses is not None:
            aravec_vec = self.get_aravec_sentence_embedding(tokens, analyses)

        # Always output fixed dim: quantum_dim + 300. Pad with zeros if AraVec unavailable.
        aravec_dim = 300
        if aravec_vec is not None:
            combined = np.concatenate([q, aravec_vec])
        else:
            # No AraVec coverage — pad with zeros to maintain constant output dim
            combined = np.concatenate([q, np.zeros(aravec_dim)])

        if not np.all(np.isfinite(combined)):
            logger.error("NaN/Inf in combined embedding. Returning zero-padded quantum only.")
            return np.pad(q, (0, len(combined) - len(q)))
        return combined
    ...
    def train(self,
        ...
        # Store circuits and binds for quantum kernel matrix computation
        self._kernel_circuits = []
        self._kernel_parameter_binds = []
        self._kernel_original_indices = []
        ...
        for i in range(len(sentences_str)): # i is the dense index for these lists
            ...
            try:
                ...
                # Store circuit + bound parameters for quantum kernel matrix computation
                param_binds_for_kernel = self._bind_parameters(
                    current_circuit_main, None,
                    current_tokens_main, current_analyses_main, current_linguistic_stream_core
                ) or {}
                self._kernel_circuits.append(current_circuit_main)
                self._kernel_parameter_binds.append(param_binds_for_kernel)
                self._kernel_original_indices.append(original_idx)

                quantum_features = self.get_enhanced_circuit_features(
                    current_circuit_main, current_tokens_main, current_analyses_main, current_linguistic_stream_core
                )
                ...
                combined_embedding = self.combine_features_with_attention(
                    ...
                    tokens=current_tokens_main, analyses=current_analyses_main
                )
    ...
    def interpret_sentence(self,
        ...
        combined_embedding = self.combine_features_with_attention(
            ...
            tokens=tokens_main, analyses=analyses_main
        )
    ...
    def evaluate_classification_accuracy(self,
        ...
        for name, clf in classifiers.items():
            try:
                ...
                # Add stratified k-fold cross-validation for reliable estimate with small datasets
                n_splits = min(5, num_classes, len(X) // max(num_classes, 1))
                cv_scores = None
                if n_splits >= 2:
                    try:
                        pipe = Pipeline([('scaler', StandardScaler()), ('clf', clf.__class__(**{k: v for k, v in clf.get_params().items()}))])
                        cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=random_state)
                        cv_scores = cross_val_score(pipe, X, y, cv=cv, scoring='accuracy')
                    except Exception as e_cv:
                        logger.warning(f"Cross-validation failed for {name}: {e_cv}")
                results_summary[name] = {
                    'classifier': name, 'accuracy': accuracy, 'report': report,
                    'num_samples_train': len(X_train), 'num_samples_test': len(X_test),
                    'num_classes': num_classes, 'embedding_type_used': embedding_type,
    ...
    def compute_quantum_kernel_matrix(self,
                                      circuits: List[Any],
                                      parameter_binds: List[Dict],
                                      shots: int = 1024) -> Optional[np.ndarray]:
        """
        Compute the quantum kernel matrix K[i,j] = P(|0...0>) from running
        circuit_i composed with circuit_j† and measuring all zeros.
        This is the standard QSVM / fidelity kernel approach.
        Returns an (n x n) symmetric positive-semidefinite kernel matrix.
        """
        if not QISKIT_AVAILABLE or self.simulator is None:
            logger.warning("Qiskit/Aer unavailable, cannot compute quantum kernel matrix.")
            return None
        n = len(circuits)
        if n == 0:
            return None
        ...
        try:
            ...
            logger.error(f"Failed to init Sampler for kernel matrix: {e}")
            return None

        K = np.eye(n)  # diagonal = 1 (same circuit overlap = 1)
        kernel_circuits = []
        idx_pairs = []
        ...
        for i in range(n):
            for j in range(i + 1, n):
                try:
                    ci = circuits[i].assign_parameters(parameter_binds[i])
                    cj = circuits[j].assign_parameters(parameter_binds[j])
                    # Remove final measurements before inversion
                    ci = ci.remove_final_measurements(inplace=False)
                    cj = cj.remove_final_measurements(inplace=False)
                    kc = ci.compose(cj.inverse())
                    kc.measure_all()
                    kernel_circuits.append(kc)
                    idx_pairs.append((i, j))
                except Exception as e:
                    logger.warning(f"Kernel circuit build failed for ({i},{j}): {e}")

        if not kernel_circuits:
            return K

        try:
            job = sampler.run(kernel_circuits)
            results = job.result()
            num_qubits = circuits[0].num_qubits
            all_zero_bitstring = 0  # integer 0 = all-zeros measurement outcome
            for idx, (i, j) in enumerate(idx_pairs):
                quasi = results.quasi_dists[idx]
                fidelity = float(quasi.get(all_zero_bitstring, 0.0))
                fidelity = max(0.0, min(1.0, fidelity))
                K[i, j] = fidelity
                K[j, i] = fidelity
        except Exception as e:
            logger.error(f"Quantum kernel matrix computation failed: {e}", exc_info=True)
            return K  # Return partial result (identity-like)

        logger.info(f"Quantum kernel matrix computed: shape={K.shape}, "
                    f"mean_off_diag={np.mean(K[K != np.diag(np.diag(K))]):.4f}")
        return K

    def evaluate_with_quantum_kernel(self,
                                     labels: List[Any],
                                     shots: int = 1024,
                                     test_size: float = 0.3,
                                     random_state: int = 42) -> Optional[Dict]:
        """
        Run kernel SVM using the quantum fidelity kernel matrix.
        Uses the stored training circuits and their parameter bindings.
        """
        if not SKLEARN_AVAILABLE:
            logger.error("scikit-learn not available for quantum kernel SVM.")
            return None
        if not hasattr(self, '_kernel_circuits') or not self._kernel_circuits:
            logger.warning("No stored kernel circuits. Run train() first with store_circuits=True.")
            return None

        circuits = self._kernel_circuits
        binds = self._kernel_parameter_binds
        n = len(circuits)
        if n < 4 or len(labels) < 4:
            logger.warning("Too few samples for quantum kernel SVM.")
            return None

        K = self.compute_quantum_kernel_matrix(circuits, binds, shots=shots)
        if K is None:
            return None
        ...
        try:
            ...
            idx_all = np.arange(n)
            train_idx, test_idx = tts(idx_all, test_size=test_size,
        ...
        try:
            ...
            from sklearn.metrics import accuracy_score, classification_report
            qk_svc = SVC(kernel='precomputed', random_state=random_state)
            qk_svc.fit(K_train, y_train)
            y_pred = qk_svc.predict(K_test)
            acc = accuracy_score(y_test, y_pred)
            report = classification_report(y_test, y_pred, zero_division=0)
            logger.info(f"Quantum Kernel SVM accuracy: {acc:.3f}")
            return {
                'classifier': 'QuantumKernelSVC',
                'accuracy': acc,
                'report': report,
                ...
                'kernel_mean_off_diag': float(np.mean(K[K != np.diag(np.diag(K))])),
            }
        except Exception as e:
            logger.error(f"Quantum kernel SVM fit/predict failed: {e}", exc_info=True)
            return None

    def evaluate_classical_aravec_baseline(self,
                                           sentences: List[Dict[str, Any]],
                                           labels: List[Any],
                                           random_state: int = 42) -> Optional[Dict]:
        """
        Run a pure classical SVM baseline using AraVec embeddings averaged over sentence tokens.
        No quantum circuits — direct Word2Vec → SVM.
        Returns classification results dict to be merged into classification_summary.
        """
        if not SKLEARN_AVAILABLE or not GENSIM_AVAILABLE:
            logger.warning("sklearn or gensim not available for classical baseline.")
            return None
        if not hasattr(self, 'embedding_model') or self.embedding_model is None:
            logger.warning("AraVec model not loaded; cannot compute classical baseline.")
            return None
        if len(sentences) != len(labels):
            logger.warning("Sentences/labels length mismatch for classical baseline.")
            return None

        X_list, y_list = [], []
        for sent_dict, label in zip(sentences, labels):
            sentence_str = sent_dict.get('sentence', '')
        ...
        if len(X_list) < 4 or len(set(y_list)) < 2:
            logger.warning("Not enough samples with AraVec coverage for classical baseline.")
            return None

        X, y = np.array(X_list), np.array(y_list)
        num_classes = len(set(y))
        results = {}
        for name, clf in [('ClassicalAraVec_GNB', GaussianNB()), ('ClassicalAraVec_SVC', SVC(random_state=random_state))]:
            try:
                n_splits = min(5, num_classes, len(X) // max(num_classes, 1))
                if n_splits >= 2:
                    ...
                    results[name] = {
                        ...
                        'embedding_type_used': 'classical_aravec_avg',
                    }
                    logger.info(f"Classical baseline {name}: CV={cv_scores.mean():.3f}±{cv_scores.std():.3f}")
                else:
                    logger.warning(f"Not enough splits for {name} CV.")
            except Exception as e:
                logger.error(f"Classical baseline {name} failed: {e}")
        ...
        if N and S and ROOT_TYPE and isinstance(N, Ty):
            ...
            try:
                ...
                if ansatz_choice.upper() == 'IQP':
                    ...
                    # SpiderAnsatz is a TENSOR ansatz, not a circuit ansatz — use IQP fallback
                    logger.warning("Spider ansatz requested for core but is a tensor ansatz (not circuit). Falling back to IQP.")
                    ...
                    ansatz_functor_for_core = StronglyEntanglingAnsatz(ob_map_core, n_layers=n_layers_strong)
                elif ansatz_choice.upper() == 'SIM14':
                    ansatz_functor_for_core = Sim14Ansatz(ob_map_core, n_layers=n_layers_iqp)
                elif ansatz_choice.upper() == 'SIM15':
                    ansatz_functor_for_core = Sim15Ansatz(ob_map_core, n_layers=n_layers_iqp)
                else:
                    ansatz_functor_for_core = IQPAnsatz(ob_map_core, n_layers=1, n_single_qubit_params=2)
    ...
    if run_classification and kernel_train_original_labels:
        ...
        if len(set(labels_for_kernel_items)) < 2:
            ...
            classification_summary = kernel.evaluate_classification_accuracy(
                labels=labels_for_kernel_items,
                embedding_type=embedding_type_for_classification
            )
            # Also run quantum kernel SVM if circuits were stored
            if kernel._kernel_circuits and len(kernel._kernel_circuits) >= 4:
                logger.info(f"[{current_set_name}] Running quantum kernel SVM on {len(kernel._kernel_circuits)} circuits...")
                qk_result = kernel.evaluate_with_quantum_kernel(
                    labels=labels_for_kernel_items,
                    shots=512  # fewer shots for speed; increase for accuracy
                )
                if qk_result and classification_summary:
                    classification_summary['QuantumKernelSVC'] = qk_result
                elif qk_result:
                    classification_summary = {'QuantumKernelSVC': qk_result}
            # Classical AraVec baseline (no quantum circuits)
            classical_baseline = kernel.evaluate_classical_aravec_baseline(
                sentences=sentences_for_current_set,
                labels=[sentences_for_current_set[i]['label'] for i in range(len(sentences_for_current_set))]
            )
            if classical_baseline and classification_summary:
                classification_summary.update(classical_baseline)
            elif classical_baseline:
                classification_summary = classical_baseline
