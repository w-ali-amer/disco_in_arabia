...
exp6.py
-------
Improved Arabic QNLP experiment combining:
  1. AraVec warm-start — circuit angles initialised from word embeddings
     instead of random noise, giving SPSA a much better starting point.
  2. 300 SPSA epochs (double exp5).
  3. Focused experiment sets:
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("qnlp_experiment_exp6.log", mode="w", encoding="utf-8"),
    ],
)
...
# ── output dir ───────────────────────────────────────────────────────────────
OUTPUT_BASE = Path("qnlp_experiment_outputs_per_set_v2") / "exp6_warmstart"
...
def _vec_for_word(word: str) -> Optional[np.ndarray]:
    """Return AraVec embedding for `word`, trying stripped article variants."""
...
def main():
    logger.info("=" * 70)
    logger.info("  Arabic QNLP Experiment 6 — AraVec Warm-Start + 300 Epochs")
    logger.info("=" * 70)
    ...
    summary_path = OUTPUT_BASE / "run_summary_exp6.json"
    ...
    # ── final table ───────────────────────────────────────────────────────
    logger.info("\n" + "=" * 70)
    logger.info("  FINAL RESULTS (exp6)")
    logger.info("=" * 70)
