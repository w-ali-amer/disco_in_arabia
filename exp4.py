...
class NumpyEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, (np.integer, np.floating, np.bool_)): # type: ignore
            return obj.item() # Use .item() for scalar numpy types
        elif isinstance(obj, np.ndarray): # type: ignore
            return obj.tolist()
        # QuantumCircuit objects are handled by removing/replacing them before serialization
        # So, this encoder doesn't need to handle them directly.
...
if __name__ == "__main__":
    ...
    # ========== START ADD/REPLACE BLOCK exp4.py - CONFIGURATION ==========
    # Lines to add/replace are typically around line 40-50 in your original exp4.py,
    # after the logger setup and before the main loop.
    
    # Define which sets from sentences.json to run
    #experiment_sets_to_run = ["MorphologyTestSet", "LexicalTestSet", "GeneralTestSet"] # Example sets
    # If you want to run all sets from the JSON:
    experiment_sets_to_run = list(experiment_sets_data.keys())


    # NEW: Configuration for arabic_morpho_lex_core.py integration
    USE_CORE_MODULE_ENHANCEMENTS = True # Master switch for using the core module features
    # Ensure "Morphology" is included if your test_root_transform_features.txt used it or if it's a key in sentences.json
    MORPHO_LEX_ENHANCED_SETS = [ "LexicalAmbiguity", "Morphology"] 
    CLASSICAL_FEATURE_DIM_CORE = 16 # Dimension for classical features from core module
    MAX_SENSES_CORE = 1             # Max senses to process from core module
    # DEBUG_CORE_MODULE = False       # Separate debug flag for the core module processing (can be added to prepare_quantum_nlp_pipeline_v8 if needed)
     
    # Choose the ansatz: 'IQP', 'Spider', 'StronglyEntangling'
    ansatz_choice_config = 'IQP' # This will be used for camel_test2 AND for the core module's internal circuit generation
    # --- Experiment Configuration ---
    n_layers_iqp = 2; n_layers_strong = 3 # Example ansatz params
    max_sentences_override_per_set = None # Process all sentences in each set by default
    run_clustering_viz_per_set = True
    run_classification_per_set = True
    classify_using_embeddings = 'combined' # Can be 'quantum' or 'combined'
    generate_per_category_pca_plots = True # For kernel's PCA plots
    run_circuit_visualization = True # For exp4.py to save circuit images
    run_heatmap_visualization = True # For exp4.py to save heatmaps
    debug_circuits_in_pipeline = False # Set to True for very detailed logs from camel_test2, False for cleaner run
    #param_per_word = 13
    all_sets_pipeline_results = {}
    all_sets_classification_summaries = {}
    all_sets_kernel_instances = {} # Store kernel instance per set if needed for later inspection
    experiment_sets_to_run = list(experiment_sets_data.keys())
    ...
    for set_name in experiment_sets_to_run:
        ...
        # Create a specific output directory for this set and ansatz combination
        ...
        if not sentences_to_process_this_run:
            logger.warning(f"No sentences to process for set '{set_name}' after potential override. Skipping."); continue

        try:
            # ========== START MODIFY BLOCK exp4.py - PIPELINE CALL ==========
            # Locate the call to prepare_quantum_nlp_pipeline_v8
            # Original might look like:
            # kernel_instance, final_results_for_set, classification_info_for_set = \
            #     prepare_quantum_nlp_pipeline_v8(...)
            #
            # Replace with/ensure it includes the new parameters:
            kernel_instance, final_results_for_set, classification_info_for_set = \
                prepare_quantum_nlp_pipeline_v8(
                    sentences_for_current_set=sentences_to_process_this_run,
                    current_set_name=set_name,
                    max_sentences_to_process=len(sentences_to_process_this_run), # Ensure this is passed if used
                    embedding_model_path=embedding_model_path,
                    ansatz_choice=ansatz_choice_config, # This is for camel_test2
                    n_layers_iqp=n_layers_iqp, 
                    n_layers_strong=n_layers_strong,
                   # cnot_ranges=cnot_ranges_config, # Pass CNOT ranges for camel_test2 if using StronglyEntangling
                    ...
                    debug_circuits=debug_circuits_in_pipeline, # For both main and core module's debug flag
                    generate_per_category_pca_plots=generate_per_category_pca_plots,
                    # Parameters for conditional core module integration:
            ...
            if kernel_instance and final_results_for_set is not None:
                all_sets_pipeline_results[set_name] = final_results_for_set
                all_sets_kernel_instances[set_name] = kernel_instance # Store the kernel for potential later use
                if classification_info_for_set:
                    all_sets_classification_summaries[set_name] = classification_info_for_set
                
                # Save results for this specific set to its directory
                ...
                save_results_to_json(
                    data={
                        ...
                        "results_data": final_results_for_set, # This list is processed by save_results_to_json
                        "classification_summary": classification_info_for_set
                    },
                    output_directory=set_specific_output_dir, filename=set_results_filename
                )

                # --- Per-set Visualizations using data from final_results_for_set ---
                if run_heatmap_visualization and kernel_instance:
                    ...
                    for res_item in final_results_for_set:
                        orig_idx = res_item.get('original_index') # Use the original index from input
                        emb_vec = res_item.get('quantum_embedding_vec') # Or 'combined_embedding_vec'
                        if orig_idx is not None and emb_vec is not None and isinstance(emb_vec, np.ndarray):
                            embeddings_for_heatmap[orig_idx] = emb_vec
                            sentences_for_heatmap_viz[orig_idx] = res_item.get('sentence', f'Sentence_{orig_idx}')
                    
                    if len(embeddings_for_heatmap) > 1:
                        heatmap_path = os.path.join(set_specific_output_dir, f"heatmap_{sanitize_filename(set_name)}_quantum.png")
                        generate_similarity_heatmap(embeddings_for_heatmap, sentences_for_heatmap_viz,
                                                    f"Similarity - {set_name} (Quantum Emb, Ansatz: {ansatz_choice_config})", heatmap_path)

                if run_circuit_visualization:
                    circuits_output_dir = os.path.join(set_specific_output_dir, "circuit_diagrams") # Changed folder name
                    os.makedirs(circuits_output_dir, exist_ok=True)
                    logger.info(f"Attempting to save circuit diagrams to: {circuits_output_dir}")
                    num_circuits_visualized = 0
                    for result_item in final_results_for_set:
                        circuit_obj = result_item.get('circuit_object') # Get the stored Qiskit circuit object
                        ...
                        if circuit_obj and QISKIT_AVAILABLE and isinstance(circuit_obj, QuantumCircuit):
                            ...
                            circuit_save_path = os.path.join(circuits_output_dir, circuit_filename)
                            visualize_circuit_matplotlib(circuit_obj, f"Set_{set_name}_Sent_{orig_idx}", circuit_save_path)
                            num_circuits_visualized +=1
                        elif not circuit_obj:
                            logger.debug(f"No circuit object found for sentence original_idx {orig_idx} in set {set_name}.")
                        elif not QISKIT_AVAILABLE:
                            logger.warning("Qiskit not available, cannot visualize circuits.") # Should only log once ideally
                    logger.info(f"Attempted to visualize {num_circuits_visualized} circuit diagrams for set '{set_name}'.")

            else:
                logger.error(f"Pipeline FAILED or returned None for set: {set_name}. No results to process or save for this set.")
        except Exception as pipeline_error:
            logger.error(f"Unhandled exception during pipeline execution for set '{set_name}': {pipeline_error}", exc_info=True)
...
#
    ...
    logger.info("\n" + "="*30 + "\nExperiment Run Finished Processing All Sets\n" + "="*30)
    consolidated_summary_filename = f"run_summary_all_sets_{sanitize_filename(ansatz_choice_config)}.json"
    # Place the consolidated summary in the ansatz-specific sub-directory inside the base output dir
    consolidated_summary_dir = os.path.join(base_output_dir, sanitize_filename(ansatz_choice_config))
    os.makedirs(consolidated_summary_dir, exist_ok=True)
    
    save_results_to_json(
        data={
            "experiment_consolidated_summary": True, "ansatz_configured": ansatz_choice_config,
            "embedding_model_path_configured": embedding_model_path,
            "sets_processed": experiment_sets_to_run,
            "classification_summary_by_set": all_sets_classification_summaries,
            "num_sets_processed": len(all_sets_pipeline_results), # Number of sets for which pipeline results were obtained
            "num_classification_results": len(all_sets_classification_summaries), # Number of sets for which classification was run
        },
        output_directory=consolidated_summary_dir, # Save in the ansatz-specific directory
        filename=consolidated_summary_filename
    )
