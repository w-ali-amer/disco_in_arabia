    ...
    if CAMEL_ANALYZER: # CAMeL Tools Integration (as in V4.4)
        try:
            ...
            if len(raw_camel) == len(stanza_word_objects):
                ...
                logger.debug(f"CAMeL/Stanza token mismatch for '{sentence}'. Using word-by-word CAMeL.")
...
def assign_discocat_types_v2_2( # Function name kept, but using V2.2.5 logic
    ...
    if is_verb_token or lemma in known_verb_lemmas:
        ...
        if not (is_N_Ty_n and is_S_Ty_s): 
            ...
            if analysis.get('upos') == "VERB":
                assigned_entity = Ty('s') if isinstance(S, Ty) else S
            else:
                assigned_entity = Ty('n') if isinstance(N, Ty) else N
    ...
    if assigned_entity is None and is_designated_predicate:
        ...
        if not (is_N_Ty_n and is_S_Ty_s):
            ...
            assigned_entity = N if is_N_Ty_n else N
        elif pos == "ADJ":
            ...
            if lemma in strongly_transitive_lemmas:
                ...
                if sentence_has_object:
                    ...
                    if obj_token_analysis:
                        ...
                        logger.debug(f"No analysis found for object index {obj_original_idx} in analyses_context.")
    ...
    if assigned_entity is None:
        if dep_rel in ['nsubj', 'obj', 'iobj', 'dobj', 'nsubj:pass', 'csubj', 'obl', 'obl:arg', 'nmod', 'appos', 'parataxis'] and pos in ["NOUN", "PROPN", "PRON", "X", "NUM"]:
            ...
            logger.debug(f"  No dependency rule matched for dep_rel='{dep_rel}', pos='{pos}'.")
    ...
    if assigned_entity is None:
        if pos in ["NOUN", "PROPN", "PRON", "NUM", "X"]:
            ...
            logger.debug(f"  No POS fallback rule matched for pos='{pos}'.")
    ...
    if isinstance(assigned_entity, Box): # Make sure Box is defined
        ...
        if changed_in_final_check:
            logger.debug(f"  Assigned entity was reset in final Box check for '{token_text}'.")
    ...
    **kwargs
    ...
    try:
        ...
        if ansatz_choice.upper() == "IQP":
            ...
            # SpiderAnsatz is a tensor ansatz — it produces tensor networks, NOT quantum circuits.
            # It cannot be converted to a Qiskit QuantumCircuit. Falling back to IQP(n=1).
            logger.warning("SPIDER ansatz is a tensor ansatz and produces no QuantumCircuit. Falling back to IQP(n=1).")
            selected_ansatz = IQPAnsatz(ob_map=ob_map, n_layers=1, n_single_qubit_params=2)
        elif ansatz_choice.upper() == "SIM14":
            selected_ansatz = Sim14Ansatz(ob_map=ob_map, n_layers=n_layers_iqp)
        elif ansatz_choice.upper() == "SIM15":
            selected_ansatz = Sim15Ansatz(ob_map=ob_map, n_layers=n_layers_iqp)
        else:
